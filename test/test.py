...
async def test_cfar_integration(dut):
    ...
    # 5. Return to Idle (Updated for Pulse Stretcher)
    dut.ui_in.value = 0
    dut.uio_in.value = 0
    
    # Wait for the 5,000,000 cycle pulse stretcher to run out, plus a few buffer cycles
    dut._log.info("Waiting for pulse stretcher to expire (this will take a moment in simulation)...")
    await ClockCycles(dut.clk, 5000005) 
    await Timer(2, units="ns") # Allow combinational logic to settle after timer drops
    
    assert dut.uo_out.value == 0x40, f"Expected '0' on display (0x40), got {hex(dut.uo_out.value)}"
    dut._log.info("Passed: System returned to idle after stretch timer")
